...
var controller = {
    ...
    save:(req:any, res:any)=>{
        ...
        if(validate_title && validate_content){
            ...
            article.save((err:Error, articleStored:any) => {
    ...
    getArticles: (req:any, res:any) => {
        ...
        let query:any = Article.find({});

        let last:any = req.params.last;
        ...
        //console.log(last);
        // find
        query.sort('-_id').exec((err:Error, articles:any) =>{
            if(err){
    ...
    getArticle: (req:any, res:any) => {
        ...
        //buscar el articulo
        Article.findById(articleId, (err:Error, article:any) => {
            if (err || !article) {
                return res.status(404).send({
                    status:"Error",
    ...
    update: (req:any, res:any) => {
        ...
        if (validateTitle && validateContent) {
            ...
            Article.findOneAndUpdate({_id: articleId}, params, {new:true}, (err:Error, articleUpdated:any) => {
                if(err){
                    return res.status(500).send({
                        status:"Error",
    ...
    delete: (req:any, res:any) => {
        ...
        let articleId = req.params.id;
        //Find and Delete
        Article.findOneAndDelete({_id: articleId}, (err:Error, articleRemoved:any) => {
            if (err) {
                return res.status(500).send({
                    status:"Error",
    ...
    upload: (req:any, res:any) => {
        ...
        if (fileExt != 'png' && fileExt != 'jpg' && fileExt != 'jpeg' && fileExt != 'gif') {
            ...
            Article.findOneAndUpdate({_id: articleId}, {image: fileName}, {new:true}, (err:Error, articleUpdated:any) => {
